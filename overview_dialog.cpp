...
Overview_Dialog::Overview_Dialog(vector<int> p_i, Civ& p, string year) : Themed_Dialog("Goverment Report"), Player(p), Flag_Image(p.Get_Texture_Path().data(), 192, 96)
{
  Player_Income = p_i;
  ...
  text = text + "\n Capital: " + string(Player.Get_Capital_Name());
  text = text + "\n Goverment: " + string(Player.Get_Active_Goverment_Name());
