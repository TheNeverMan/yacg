...
bool Scaled_Pixbuf::Is_Size_Different(int x_s, int y_s)
{
  bool out = true;
...
bool Scaled_Pixbuf::Is_Texture_Different(string t_p)
{
  bool out = true;
  Image_Path tmp(t_p);
...
bool Scaled_Pixbuf::Is_Color_Different(guint32 color)
{
  bool out = true;
  if(color == Main_Color)
    out = false;
...
void Scaled_Pixbuf::Set_Texture_And_Size(string t_p, int x_s, int y_s)
{
  Set_Texture(t_p);
  Resize(x_s, y_s);
}

void Scaled_Pixbuf::Set_Texture(string t_p)
{
...
Glib::RefPtr<Gdk::Pixbuf> Scaled_Pixbuf::Get_Pixbuf()
{
  return Main_Pixbuf;
}
